...
class MerossShutterDevice(RollerShutterTimerMixin, BaseDevice):
    """
    Type hints helper
    """
    pass
...
class ShutterEntityWrapper(SwitchEntity):
    """Wrapper class to adapt the Meross shutters into the Homeassistant platform"""

    def __init__(self, device: MerossShutterDevice, channel: int):
        self._device = device

        # If the current device has more than 1 channel, we need to setup the device name and id accordingly
        self._id = calculate_shutter_id(device.internal_id, channel)
    ...
    async def async_stop(self, **kwargs) -> None:
        await self._device.async_stop(channel=self._channel_id)

    def open(self, **kwargs: Any) -> None:
        self.hass.async_add_executor_job(self.async_open, **kwargs)

    def close(self, **kwargs: Any) -> None:
        self.hass.async_add_executor_job(self.async_close, **kwargs)

    def stop(self, **kwargs: Any) -> None:
        self.hass.async_add_executor_job(self.async_stop, **kwargs)
...
async def _add_entities(hass: HomeAssistant, devices: Iterable[BaseDevice], async_add_entities):
    ...
    # Identify all the devices that expose the Position capabilities
